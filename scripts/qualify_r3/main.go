// qualify_r3/main.go
//
// Usage (run from scripts/qualify_r3/):
...
//   - Sets isQualifiedForR3 = true  for teams whose ID appears in the CSV.
//   - Sets isQualifiedForR3 = false for all other teams.
...
func main() {
	...
	log.Printf("[qualify_r3] Loaded %d qualified team ID(s) from %q", len(qualifiedIDs), *csvPath)
	...
	for {
		...
		batch.Set(doc.Ref,
			map[string]interface{}{"isQualifiedForR3": isQualified},
			firestore.MergeAll,
		)
		...
		if batchCount == 500 {
			...
			batch = client.Batch()
			batchCount = 0
			log.Printf("[qualify_r3] Committed intermediate batch (500 writes)...")
		}
	}
	...
	log.Printf("[qualify_r3] Done. %d total teams updated — %d true, %d false.",
